...
ds_bp = Blueprint('DEATHSTARE', __name__)
...
@ds_bp.route('/camera_feed/<int:Number>')
def camera_feed(Number):
    logging.debug("DEATHSTARE.camera_feed")

    if len(current_app.Cameras) <= Number:
        the_camera = current_app.Cameras[-1]
    else:
        the_camera = current_app.Cameras[Number]
...
def returnToIndex():
    ...
    for i in range(len(current_app.Cameras)):
        current_app.Cameras[i].clear_rectangle()
...
@ds_bp.route('/picker_image')
def picker_image():
    logging.debug("DEATHSTARE.picker_image")

    the_camera = current_app.Cameras[0]
...
@ds_bp.route("/pick")
def pick():
    current_app.Cameras[0].get_frame()
    col = rr1716.Vision.averageColor(current_app.Cameras[0].get_color(), __COLOR_PICK_RANGE__)
    print("color: ", col) 
    #write to file
...
@ds_bp.route("/pick_cone")
def pick_cone():
    current_app.Cameras[0].get_frame()
    col = rr1716.Vision.averageColor(current_app.Cameras[0].get_frame(), __COLOR_PICK_RANGE__)
